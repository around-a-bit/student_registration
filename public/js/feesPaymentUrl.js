const initialUrl = "http://127.0.0.1:8000/feesPayments";

function urlFilterFees(page = 1) {
    let queryString = '';

    const semester_id = $('#semester_id').val();
    const degree_id = $('#degree_id').val();
    ...
    if(!search_term){
        ...
        if (degree_id) queryString += 'degree_id=' + degree_id + '&';
        if (date_from) queryString += 'date_from=' + date_from + '&';
        if (date_to) queryString += 'date_to=' + date_to + '&';
        }
