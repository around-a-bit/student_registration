    ...
    function openTab(event, tabName) {
        var currentTab = document.querySelector(".tabcontent:not([style='display: none;'])").id;
        var inputs = document.querySelectorAll(`#${currentTab} input[required], #${currentTab} select[required]`);
        
        let tabList = ["basic-details", "course-selection", "address", "upload-document"];
        let currentIndex = tabList.indexOf(currentTab);
        let targetIndex = tabList.indexOf(tabName);
        
        if (targetIndex > currentIndex) { // Moving forward
    ...
    //     var activeTab = localStorage.getItem("activeTab") || "basic-details";
    //     openTab({ currentTarget: document.querySelector(`.nav-link[onclick*="${activeTab}"]`) }, activeTab);
    // };
